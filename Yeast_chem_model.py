...
class Yeast(object):
    ...
    def __init__(self, GLC, PEP, PYR, ETHANOL, NAD, NADH, PYK1_in, PYK1_ac, PYK2_in, PYK2_ac, k_lst, regulation=True, wt=True):
        """[summary]
        ...
        Arguments:
            ...
            NAD {int} -- the initial number of this species in a yeast.
            NADH {int} -- the initial number of this species in a yeast.
        ...
        self.NAD = Species(NAD, energetic=True) 
        self.NADH = Species(NADH, energetic=True)
    ...
    def init_reactions(self):
        self.reactants = {0: [self.GLC, self.NAD],\
                          1: [self.PEP],\
                          2:[self.PYR, self.NADH, self.PYK1_ac],\
                          3:[self.ETHANOL, self.NAD],\
                          4:[self.PYK2_ac, self.PYR],
                          5:[self.PYK1_in] ,\
                          6:[self.PYK2_in],\
                          7:[self.PYK1_ac],\
                          8:[self.PYK2_ac],\
        ...
        self.products = {0: [self.PEP, self.NADH], \
                         1: [self.PYR, self.ATP], \
                         2:[self.ETHANOL, self.NAD],\
                         3:[self.PYR, self.NADH], \
                         4:[self.ATP_r], \
                         5:[self.PYK1_ac],\
                         6:[self.PYK2_ac],  \
                         7:[self.PYK1_in] , \
    ...
    def update_history(self):
        ...
        self.NAD_t.append(self.NAD.n) 
        self.NADH_t.append(self.NADH.n) 
    ...
    def stats(self):
        ...
        stats['NAD_t'] = self.NAD_t 
        stats['NADH_t'] = self.NADH_t
        stats['PYK1_ac_t'] = self.PYK1_ac_t
        stats['PYK2_ac_t'] = self.PYK2_ac_t
        stats['time'] = self.time
        return stats

    def get_propensity(self, ind, reactants, products, k):
        if self.regulation == True:
            if ind == 5: #PYK1 activation
                prop = k * (1e-3* self.GLC.n**2)/(30.**2 + self.GLC.n**2) * reactants[0].n
            elif ind == 6: #PYK2 activation
                prop =  k * 1e-3 * reactants[0].n
            elif ind == 7: #PYK1 inactivation
                prop = k * 1e-2*(1.* self.PEP.n**2)/(1.**2 + self.PEP.n**2) * reactants[0].n  
            elif ind == 8: #PYK2 inactivation
                prop = k * 1e-3 * (1. * self.GLC.n**2)/(30.**2 + self.GLC.n**2) * reactants[0].n
            else:
                prop = k
                for i in reactants:
                    prop *= i.n
            ...
            if ind == 5: #PYK1 activation
                if self.strategy == 'ferm':
                    prop = 1. * self.PYK1_in.n
                else:
                    prop = 0.
            #PYK2 activation
            elif ind == 6: 
                if self.strategy == 'resp':
                    prop = 1. * self.PYK2_in.n
                else:
                    prop = 0.
            else:
                prop = k
    ...
    def next_reaction_method(self):
        ...
        for i in range(len(props)):
            props[i] = self.get_propensity(i, self.reactants[i], self.products[i], self.k[i])
    ...
    def execute_reaction(self, min_time, min_time_ind):
        if min_time == np.inf:
            ...
            for i in self.reactants[min_time_ind]:
                if (min_time_ind == 2 or min_time_ind == 4) and i.regulator:
                    pass
                else:
                    if min_time_ind == 0:
                        if i.energetic:
    ...
    def simulate(self, T, fixed_source=True):
        ...
        Keyword Arguments:
            fixed_source {bool} -- Indicate if the glucose remins constant in 
                                   the environment (default: {False})
        """
        self.update_history()
        for i in range(T):
            a, b = self.next_reaction_method()
            self.execute_reaction(a, b)
            if fixed_source:
                self.GLC.set_n(self.init_GLC_count)
            self.update_history()
...
class Population(object):
    ...
    def __init__(self, cell, N, fixed_source=True, duplication=False):
        ...
        Keyword Arguments:
            fixed_source {bool} -- If True the amount of glucose remains 
                                   fixed during simulation (default: {False})
        ...
        for i in range(N):
            ...
            if self.init_cell.regulation == False:
                ...
                if rand >= self.init_cell.strat_prop:
    ...
    def duplicate_cell(self, fission_threshold= 25.):
        new_cells = []
        for i in self.pop:
            if i.ATP.n + i.ATP_r.n > fission_threshold:
                progeny = deepcopy(i)
                if i.ATP_r.n == 0:
                    temp = i.ATP.n
                    i.ATP.n = temp - fission_threshold
                else:
                    temp = i.ATP_r.n
                    i.ATP_r.n = temp - fission_threshold
                progeny.ATP.n = 0.
                progeny.ATP_r.n = 0.
                new_cells.append(progeny)
        self.pop.extend(new_cells)
        
    def simulate_step(self):
        times = np.zeros(self.N)
        times_ind = np.zeros(self.N)
        for i in range(self.N):
            t, t_ind = self.pop[i].next_reaction_method()
            times[i] = t
            times_ind[i] = t_ind
        ...
        for i in sorted_times:
            ...
            if not self.fixed_source:
                ...
                if new_pool < 0:
                    new_pool = 0
                self.glc_pool = deepcopy(new_pool)
                if diff != 0:
                    for j in sorted_times[i:]:
                        self.pop[j].GLC.set_n(self.glc_pool)
            else:
                for j in sorted_times[i:]:
                        self.pop[j].GLC.set_n(self.glc_pool)
